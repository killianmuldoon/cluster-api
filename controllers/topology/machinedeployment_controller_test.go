...
func TestMachineDeploymentReconciler_ReconcileDelete(t *testing.T) {
	...
	mdBT := builder.NewBootstrapTemplateBuilder(metav1.NamespaceDefault, "mdBT").Build()
	mdIMT := builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "mdIMT").Build()
	...
	t.Run("Should delete infra template of a MachineDeployment without a bootstrap template", func(t *testing.T) {
		...
		mdWithoutBootstrapTemplateIMT := builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "mdWithoutBootstrapTemplateIMT").Build()
		mdWithoutBootstrapTemplate := builder.NewMachineDeploymentBuilder(metav1.NamespaceDefault, "mdWithoutBootstrapTemplate").
	...
	t.Run("Should not delete templates of a MachineDeployment when they are still in use in a MachineSet", func(t *testing.T) {
		...
		ms := builder.NewMachineSetBuilder(md.Namespace, "ms").
			WithBootstrapTemplate(mdBT).
			WithInfrastructureTemplate(mdIMT).
