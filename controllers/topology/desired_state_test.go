...
func TestComputeInfrastructureCluster(t *testing.T) {
	...
	infrastructureClusterTemplate := builder.NewInfrastructureClusterTemplateBuilder(metav1.NamespaceDefault, "template1").
...
func TestComputeControlPlaneInfrastructureMachineTemplate(t *testing.T) {
	...
	infrastructureMachineTemplate := builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "template1").
		WithSpecFields(map[string]interface{}{"spec.template.spec.fakeSetting": true}).
		Build()
	clusterClass := builder.NewClusterClassBuilder(metav1.NamespaceDefault, "class1").
	...
	t.Run("If there is already a reference to the infrastructureMachineTemplate, it preserves the reference name", func(t *testing.T) {
		...
		// current cluster objects for the test scenario
		currentInfrastructureMachineTemplate := builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "cluster1-template1").Build()
...
func TestComputeControlPlane(t *testing.T) {
	...
	controlPlaneTemplate := builder.NewControlPlaneTemplateBuilder(metav1.NamespaceDefault, "template1").
		WithSpecFields(map[string]interface{}{"spec.template.spec.fakeSetting": true}).
		Build()
	clusterClass := builder.NewClusterClassBuilder(metav1.NamespaceDefault, "class1").
		WithControlPlaneMetadata(labels, annotations).
	...
	t.Run("Generates the ControlPlane from the template and adds the infrastructure machine template if required", func(t *testing.T) {
		...
		// templates and ClusterClass
		infrastructureMachineTemplate := builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "template1").Build()
		clusterClass := builder.NewClusterClassBuilder(metav1.NamespaceDefault, "class1").
			WithControlPlaneMetadata(labels, annotations).
	...
	t.Run("Should choose the correct version for control plane", func(t *testing.T) {
		...
		tests := []struct {
			...
			},
			{
				...
				currentControlPlane: builder.NewControlPlaneBuilder("test1", "cp1").
...
func TestComputeControlPlaneVersion(t *testing.T) {
	...
	// is false.
	machineDeploymentStable := builder.NewMachineDeploymentBuilder("test-namespace", "md1").
		...
		Build()
	machineDeploymentRollingOut := builder.NewMachineDeploymentBuilder("test-namespace", "md2").
		WithGeneration(int64(1)).
		WithReplicas(int32(2)).
		WithStatus(clusterv1.MachineDeploymentStatus{
			ObservedGeneration: 2,
	...
	tests := []struct {
		...
		},
		{
			...
			topologyVersion: "v1.2.3",
			controlPlaneObj: builder.NewControlPlaneBuilder("test1", "cp1").
				WithSpecFields(map[string]interface{}{
					"spec.version":  "v1.2.2",
...
func TestComputeCluster(t *testing.T) {
	...
	// generated objects
	infrastructureCluster := builder.NewInfrastructureClusterBuilder(metav1.NamespaceDefault, "infrastructureCluster1").
		Build()
	controlPlane := builder.NewControlPlaneBuilder(metav1.NamespaceDefault, "controlplane1").
		Build()
...
func TestComputeMachineDeployment(t *testing.T) {
	workerInfrastructureMachineTemplate := builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "linux-worker-inframachinetemplate").
		WithSpecFields(map[string]interface{}{"spec.template.spec.fakeSetting": true}).
		Build()
	workerBootstrapTemplate := builder.NewBootstrapTemplateBuilder(metav1.NamespaceDefault, "linux-worker-bootstraptemplate").
		Build()
	labels := map[string]string{"fizz": "buzz", "foo": "bar"}
	annotations := map[string]string{"annotation-1": "annotation-1-val"}

	md1 := builder.NewMachineDeploymentClassBuilder(metav1.NamespaceDefault, "class1").
	...
	fakeClass := builder.NewClusterClassBuilder(metav1.NamespaceDefault, "class1").
	...
	t.Run("Should choose the correct version for machine deployment", func(t *testing.T) {
		controlPlaneStable123 := builder.NewControlPlaneBuilder("test1", "cp1").
			WithSpecFields(map[string]interface{}{
				"spec.version":  "v1.2.3",
				"spec.replicas": int64(2),
			}).
		...
		machineDeploymentStable := builder.NewMachineDeploymentBuilder("test-namespace", "md-1").
			WithGeneration(1).
			WithReplicas(2).
			WithStatus(clusterv1.MachineDeploymentStatus{
				ObservedGeneration: 2,
		...
		machineDeploymentRollingOut := builder.NewMachineDeploymentBuilder("test-namespace", "md-1").
			WithGeneration(1).
			WithReplicas(2).
			WithStatus(clusterv1.MachineDeploymentStatus{
				ObservedGeneration: 2,
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if tt.currentMDVersion != nil {
					...
					md := builder.NewMachineDeploymentBuilder("test-namespace", "big-pool-of-machines").
						WithGeneration(1).
						WithReplicas(2).
...
func TestComputeMachineDeploymentVersion(t *testing.T) {
	controlPlaneStable122 := builder.NewControlPlaneBuilder("test1", "cp1").
		WithSpecFields(map[string]interface{}{
			"spec.version":  "v1.2.2",
			"spec.replicas": int64(2),
		}).
	...
	controlPlaneStable123 := builder.NewControlPlaneBuilder("test1", "cp1").
		...
		Build()
	controlPlaneUpgrading := builder.NewControlPlaneBuilder("test1", "cp1").
		WithSpecFields(map[string]interface{}{
			"spec.version": "v1.2.3",
		}).
		WithStatusFields(map[string]interface{}{
		...
		Build()
	controlPlaneScaling := builder.NewControlPlaneBuilder("test1", "cp1").
		WithSpecFields(map[string]interface{}{
			"spec.version":  "v1.2.3",
			"spec.replicas": int64(2),
		}).
		...
		Build()
	controlPlaneDesired := builder.NewControlPlaneBuilder("test1", "cp1").
		WithSpecFields(map[string]interface{}{
			"spec.version": "v1.2.3",
		}).
		Build()
	...
	machineDeploymentStable := builder.NewMachineDeploymentBuilder("test-namespace", "md-1").
		...
		Build()
	machineDeploymentRollingOut := builder.NewMachineDeploymentBuilder("test-namespace", "md-2").
		WithGeneration(1).
		WithReplicas(2).
		WithStatus(clusterv1.MachineDeploymentStatus{
			ObservedGeneration: 2,
	...
	tests := []struct {
		...
		},
		{
			...
			currentMachineDeploymentState: &scope.MachineDeploymentState{Object: builder.NewMachineDeploymentBuilder("test1", "md-current").WithVersion("v1.2.2").Build()},
...
func TestTemplateToObject(t *testing.T) {
	template := builder.NewInfrastructureClusterTemplateBuilder(metav1.NamespaceDefault, "infrastructureClusterTemplate").
		WithSpecFields(map[string]interface{}{"spec.template.spec.fakeSetting": true}).
		Build()
	cluster := &clusterv1.Cluster{
		ObjectMeta: metav1.ObjectMeta{
