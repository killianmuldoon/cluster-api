...
func TestMachineSetReconciler_ReconcileDelete(t *testing.T) {
	...
	msBT := builder.NewBootstrapTemplateBuilder(metav1.NamespaceDefault, "msBT").Build()
	msIMT := builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "msIMT").Build()
	...
	t.Run("Should delete infra template of a MachineSet without a bootstrap template", func(t *testing.T) {
		...
		msWithoutBootstrapTemplateIMT := builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "msWithoutBootstrapTemplateIMT").Build()
		msWithoutBootstrapTemplate := builder.NewMachineSetBuilder(metav1.NamespaceDefault, "msWithoutBootstrapTemplate").
	...
	t.Run("Should not delete templates of a MachineSet when they are still in use in another MachineSet", func(t *testing.T) {
		...
		anotherMS := builder.NewMachineSetBuilder(metav1.NamespaceDefault, "anotherMS").
			WithBootstrapTemplate(msBT).
			WithInfrastructureTemplate(msIMT).
			WithLabels(map[string]string{
				clusterv1.MachineDeploymentLabelName: mdName,
