...
func TestGetReference(t *testing.T) {
	...
	controlPlaneTemplate := builder.NewControlPlaneTemplateBuilder(metav1.NamespaceDefault, "controlplanetemplate1").Build()
	...
	workerBootstrapTemplate := builder.NewBootstrapTemplateBuilder(metav1.NamespaceDefault, "workerbootstraptemplate1").Build()
...
func TestCalculateTemplatesInUse(t *testing.T) {
	t.Run("Calculate templates in use with regular MachineDeployment and MachineSet", func(t *testing.T) {
		...
		md := builder.NewMachineDeploymentBuilder(metav1.NamespaceDefault, "md").
			WithBootstrapTemplate(builder.NewBootstrapTemplateBuilder(metav1.NamespaceDefault, "mdBT").Build()).
			WithInfrastructureTemplate(builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "mdIMT").Build()).
			Build()
		ms := builder.NewMachineSetBuilder(metav1.NamespaceDefault, "ms").
			WithBootstrapTemplate(builder.NewBootstrapTemplateBuilder(metav1.NamespaceDefault, "msBT").Build()).
			WithInfrastructureTemplate(builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "msIMT").Build()).
			Build()
	...
	t.Run("Calculate templates in use with MachineDeployment and MachineSet without BootstrapTemplate", func(t *testing.T) {
		...
		mdWithoutBootstrapTemplate := builder.NewMachineDeploymentBuilder(metav1.NamespaceDefault, "md").
			WithInfrastructureTemplate(builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "mdIMT").Build()).
			Build()
		msWithoutBootstrapTemplate := builder.NewMachineSetBuilder(metav1.NamespaceDefault, "ms").
			WithInfrastructureTemplate(builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "msIMT").Build()).
			Build()
	...
	t.Run("Calculate templates in use with MachineDeployment and MachineSet ignore templates when resources in deleting", func(t *testing.T) {
		...
		mdInDeleting := builder.NewMachineDeploymentBuilder(metav1.NamespaceDefault, "md").
			WithInfrastructureTemplate(builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "mdIMT").Build()).
			Build()
		mdInDeleting.SetDeletionTimestamp(&deletionTimeStamp)

		msInDeleting := builder.NewMachineSetBuilder(metav1.NamespaceDefault, "ms").
			WithInfrastructureTemplate(builder.NewInfrastructureMachineTemplateBuilder(metav1.NamespaceDefault, "msIMT").Build()).
			Build()
